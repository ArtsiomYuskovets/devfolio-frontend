...
export function ProjectsFeedList({ filters }: ProjectsFeedListProps) {
  ...
  const isInitialLoading =
    isLoading && projects.length === 0 && lastNonEmptyProjectsRef.current.length === 0;
  const isWaitingForFilteredResults =
    (isLoading || isFetching) &&
    projects.length === 0 &&
    lastNonEmptyProjectsRef.current.length > 0;
  const displayProjects =
    projects.length > 0
      ? projects
      : isWaitingForFilteredResults
        ? lastNonEmptyProjectsRef.current
        : projects;

  const showEmptyState =
    !isLoading && !isFetching && projects.length === 0 && !isWaitingForFilteredResults;
  const showPagination = !isInitialLoading && (displayProjects.length > 0 || canGoPrev);

  let listBody: React.ReactNode;

  if (isInitialLoading) {
    listBody = (
      <p className={styles["projects-feed__list-placeholder"]}>
        Загрузка проектов…
      </p>
    );
  } else if (isError && displayProjects.length === 0) {
    listBody = (
      <p className={styles["projects-feed__list-placeholder"]} role="alert">
    ...
    listBody = displayProjects.map((project) => {
